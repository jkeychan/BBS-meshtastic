...
def handle_help_command(sender_id, interface, menu_name=None):
    if menu_name:
        update_user_state(
            sender_id, {'command': 'MENU', 'menu': menu_name, 'step': 1})
        if menu_name == 'bbs':
        ...
        if choice == 'x':
            ...
            for period, seconds in timeframes.items():
                if seconds is None:
                    ...
                    total_nodes = sum(1 for node in interface.nodes.values() if node.get(
                        'lastHeard') is not None and node['lastHeard'] >= time_limit)
            ...
            response = "Hardware Models:\n" + \
                "\n".join([f"{model}: {count}" for model,
                          count in hw_models.items()])
            send_message(response, sender_id, interface)
            handle_stats_command(sender_id, interface)
        elif choice == 'r':
            roles = {}
            ...
            response = "Roles:\n" + \
                "\n".join([f"{role}: {count}" for role,
                          count in roles.items()])
            send_message(response, sender_id, interface)
            handle_stats_command(sender_id, interface)
...
def handle_bb_steps(sender_id, message, step, state, interface, bbs_nodes):
    ...
    if step == 1:
        ...
        if message.lower() == 'r':
            ...
            if bulletins:
                send_message(
                    f"Select a bulletin number to view from {board_name}:", sender_id, interface)
                for bulletin in bulletins:
                    send_message(
                        f"[{bulletin[0]}] {bulletin[1]}", sender_id, interface)
                update_user_state(
                    sender_id, {'command': 'BULLETIN_READ', 'step': 3, 'board': board_name})
            else:
                send_message(
                    f"No bulletins in {board_name}.", sender_id, interface)
            ...
            if board_name.lower() == 'urgent':
                ...
                if allowed_nodes and node_id not in allowed_nodes:
                    send_message(
                        "You don't have permission to post to this board.", sender_id, interface)
                    handle_bb_steps(sender_id, 'e', 1, state,
                                    interface, bbs_nodes)
                    return
            send_message(
                "What is the subject of your bulletin? Keep it short.", sender_id, interface)
            update_user_state(
        ...
        subject = message
        send_message(
            "Send the contents of your bulletin. Send a message with END when finished.", sender_id, interface)
        update_user_state(sender_id, {'command': 'BULLETIN_POST_CONTENT', 'step': 5,
                          'board': state['board'], 'subject': subject, 'content': ''})
        ...
        if message.lower() == "end":
            ...
            if node_info is None:
                send_message(
                    "Error: Unable to retrieve your node information.", sender_id, interface)
                update_user_state(sender_id, None)
                return
            sender_short_name = node_info['user'].get(
                'shortName', f"Node {sender_id}")
...
def handle_mail_steps(sender_id, message, step, state, interface, bbs_nodes):
    ...
    if step == 1:
        ...
        if choice == 'r':
            ...
            if mail:
                send_message(
                    f"You have {len(mail)} mail messages. Select a message number to read:", sender_id, interface)
                for msg in mail:
                    send_message(
            ...
            send_message(
                "What is the Short Name of the node you want to leave a message for?", sender_id, interface)
        ...
        try:
            ...
            update_user_state(sender_id, {'command': 'MAIL', 'step': 4, 'mail_id': mail_id,
                              'unique_id': unique_id, 'sender': sender, 'subject': subject, 'content': content})
        except TypeError:
            logging.info(
                f"Node {sender_id} tried to access non-existent message")
        ...
        if not nodes:
            ...
            send_message(
                "There are multiple nodes with that short name. Which one would you like to leave a message for?", sender_id, interface)
            for i, node in enumerate(nodes):
                send_message(f"[{i}] {node['longName']}", sender_id, interface)
            update_user_state(
                sender_id, {'command': 'MAIL', 'step': 6, 'nodes': nodes})
        ...
        if message.lower() == "d":
            ...
            send_message("The message has been deleted 🗑️",
                         sender_id, interface)
            update_user_state(sender_id, None)
        elif message.lower() == "r":
            sender = state['sender']
            send_message(
                f"Send your reply to {sender} now, followed by a message with END", sender_id, interface)
            update_user_state(sender_id, {
                              'command': 'MAIL', 'step': 7, 'reply_to_mail_id': state['mail_id'], 'subject': f"Re: {state['subject']}", 'content': ''})
        else:
            send_message(
        ...
        update_user_state(sender_id, {'command': 'MAIL', 'step': 7,
                          'recipient_id': state['recipient_id'], 'subject': subject, 'content': ''})
        ...
        if message.lower() == "end":
            if 'reply_to_mail_id' in state:
                recipient_id = get_sender_id_by_mail_id(
                    state['reply_to_mail_id'])  # Get the sender ID from the mail ID
            else:
            ...
            unique_id = add_mail(get_node_id_from_num(
                sender_id, interface), sender_short_name, recipient_id, subject, content, bbs_nodes, interface)
            send_message(
        ...
        if message.lower() == "y":
            ...
            send_message("Okay, feel free to send another command.",
                         sender_id, interface)
            update_user_state(sender_id, None)
...
def handle_channel_directory_steps(sender_id, message, step, state, interface):
    ...
    if step == 1:
        ...
        if choice.lower() == 'x':
            ...
            send_message("Name your channel for the directory:",
                         sender_id, interface)
            update_user_state(
                sender_id, {'command': 'CHANNEL_DIRECTORY', 'step': 3})
        ...
        send_message("Send a message with your channel URL or PSK:",
                     sender_id, interface)
        update_user_state(sender_id, {
        ...
        send_message(
            f"Your channel '{channel_name}' has been added to the directory.", sender_id, interface)
        handle_channel_directory_command(sender_id, interface)
...
def handle_send_mail_command(sender_id, message, interface, bbs_nodes):
    try:
        ...
        send_message(
            f"Mail has been sent to {recipient_name}.", sender_id, interface)
        ...
        send_message("Error processing send mail command.",
                     sender_id, interface)
...
def handle_check_mail_command(sender_id, interface):
    try:
        ...
        update_user_state(
            sender_id, {'command': 'CHECK_MAIL', 'step': 1, 'mail': mail})

    except Exception as e:
        logging.error(f"Error processing check mail command: {e}")
        send_message("Error processing check mail command.",
                     sender_id, interface)
...
def handle_read_mail_command(sender_id, message, state, interface):
    try:
        ...
        if message_number < 0 or message_number >= len(mail):
            send_message("Invalid message number. Please try again.",
                         sender_id, interface)
            return
        ...
        send_message(
            "What would you like to do with this message?\n[K]eep  [D]elete  [R]eply", sender_id, interface)
        update_user_state(sender_id, {'command': 'CHECK_MAIL', 'step': 2, 'mail_id': mail_id,
                          'unique_id': unique_id, 'sender': sender, 'subject': subject, 'content': content})
        ...
        send_message("Error processing read mail command.",
                     sender_id, interface)
...
def handle_delete_mail_confirmation(sender_id, message, state, interface, bbs_nodes):
    try:
        ...
        send_message("Error processing delete mail confirmation.",
                     sender_id, interface)
...
def handle_post_bulletin_command(sender_id, message, interface, bbs_nodes):
    try:
        ...
        send_message(
            f"Your bulletin '{subject}' has been posted to {board_name}.", sender_id, interface)
        ...
        send_message("Error processing post bulletin command.",
                     sender_id, interface)
...
def handle_check_bulletin_command(sender_id, message, interface):
    try:
        ...
        boards = {0: "General", 1: "Info",
                  2: "News", 3: "Urgent"}  # list of boards
        # get board name from quick command and capitalize it
        board_name = parts[1].strip().capitalize()
        board_name = boards[next(key for key, value in boards.items(
        ) if value == board_name)]  # search for board name in list
        ...
        update_user_state(sender_id, {'command': 'CHECK_BULLETIN',
                          'step': 1, 'board_name': board_name, 'bulletins': bulletins})

    except Exception as e:
        logging.error(f"Error processing check bulletin command: {e}")
        send_message("Error processing check bulletin command.",
                     sender_id, interface)
...
def handle_read_bulletin_command(sender_id, message, state, interface):
    try:
        ...
        sender, date, subject, content, unique_id = get_bulletin_content(
            bulletin_id)
        response = f"Date: {date}\nFrom: {sender}\nSubject: {subject}\n\n{content}"
        send_message(response, sender_id, interface)
        ...
        send_message("Error processing read bulletin command.",
                     sender_id, interface)
...
def handle_post_channel_command(sender_id, message, interface):
    try:
        ...
        send_message(
            f"Channel '{channel_name}' has been added to the directory.", sender_id, interface)

    except Exception as e:
        logging.error(f"Error processing post channel command: {e}")
        send_message("Error processing post channel command.",
                     sender_id, interface)
...
def handle_check_channel_command(sender_id, interface):
    try:
        channels = get_channels()
        if not channels:
            send_message("No channels available in the directory.",
                         sender_id, interface)
            return
        ...
        send_message("Error processing check channel command.",
                     sender_id, interface)
...
def handle_read_channel_command(sender_id, message, state, interface):
    try:
        ...
        if message_number < 0 or message_number >= len(channels):
            send_message("Invalid channel number. Please try again.",
                         sender_id, interface)
            return
        ...
        send_message("Error processing read channel command.",
                     sender_id, interface)
...
def handle_list_channels_command(sender_id, interface):
    try:
        ...
        send_message("Error processing list channels command.",
                     sender_id, interface)
